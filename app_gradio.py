...
COLOR_BG = "#F4F6F8"         # page background — soft, muted blue-gray (not saturated)
COLOR_CARD = "#FFFFFF"       # cards/panels
COLOR_BLUE = "#3B6E96"       # primary buttons / accents — muted steel blue, not neon
COLOR_BLUE_HOVER = "#2F5A7C"
COLOR_POSITIVE = "#2E8B57"   # muted sea green
COLOR_NEGATIVE = "#C0392B"   # muted brick red
COLOR_NEUTRAL = "#D4A017"    # muted amber/gold
COLOR_BORDER = "#E2E8F0"
COLOR_SUBJECTIVE = "#3B6E96"     # reuse accent blue — distinct from sentiment colors
COLOR_OBJECTIVE = "#A9B4C0"      # muted slate gray — distinct from sentiment colors
...
/* ---- Cards / panels: groups & blocks get white bg, rounded corners, faint border ---- */
.gr-group, .block {{
...
/* ---- Inputs: rounded, subtle border, gentle focus outline ---- */
...
with gr.Blocks(
    ...
    with gr.Group(visible=False) as main_app:
        ...
        with gr.Tabs():
            # ---------------- Tab 1: YouTube comments (headline feature) ----------------
            with gr.Tab("📺 YouTube Comments"):
                gr.Markdown(
                    "## Analyze Sentiment Across a YouTube Video's Comments\n"
                    "Paste a video link, add your YouTube Data API v3 key, and get a full sentiment "
                    "breakdown of real audience reactions — sentiment distribution, subjectivity, "
                    "confidence, top words, and a trend chart over time. Get a free API key from "
            ...
            # ---------------- Tab 2: Single prediction + explainability ----------------
